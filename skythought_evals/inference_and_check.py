...
def perform_inference_and_check(
    ...
    if not len(remaining_data):
        ...
        return
    print('show template:', model_config.system_prompt, model_config.user_template)
...
def main():
    ...
    parser.add_argument(
        "--max_tokens", type=int, default=32768, help="Max tokens for the model." # 32768 16384
    )
    parser.add_argument(
        "--split",
        type=str,
        ...
        if args.use_ray:
            ...
            llm = (
                ...
                if args.model.startswith("openai")
                    ...
                    #  max_model_len=max_tokens,rope_scaling={"rope_type": "linear", "factor": 2.0}
                )
                # speculative model
            )
        if args.inference:
